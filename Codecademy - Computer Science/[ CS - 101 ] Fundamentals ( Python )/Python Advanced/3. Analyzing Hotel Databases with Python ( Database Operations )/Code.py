...
lead_average_can = sum/len(lead_time_can)
print(lead_average_can)
...
lead_average = sum/len(lead_time)
print(lead_average)

# Task 13: Retrieve special_requests rows where the bookings were canceled
total_amt_can = cur.execute('''SELECT special_requests FROM bra_customers WHERE is_cancelled = 1;''').fetchall()

# Task 14: Find total speacial requests for those who canceled and print results
sum_amt_can = 0
for i in total_amt_can:
  sum_amt_can += i[0]

# Task 15: Retrieve special_requests rows where the bookings were not canceled
total_amt = cur.execute('''SELECT special_requests FROM bra_customers WHERE is_cancelled = 0;''').fetchall()
